(function(window,document, screen, navigator){	
    ...
    if (typeof jap === "undefined") {
    	...
    	jap = {
	        ...
	        jumpAppEnable : 0, //是否允许M页与APP做数据通信(自动向appurl追加cls参数), 0:不允许 1:允许
	        ...
	        topic: '', // 主题参数
	        anchorpvflag: 0, // 用anchor的方式记录pv, 0:关闭 1:开启
	        autoLogPv: 1 // 自动上报pv
	    };
    }
...
var store = new Store();
	...
	var jsonp = (function(window){
        ...
        account = (typeof jap.account != 'undefined') ? jap.account : store.getItem('pin'),
        pinId = store.getItem('pinId') || '',
        topic = jap.topic || 'traffic-oth',
        adsCookieName = jap.adsCookieName || '',
        ads = '',
        pre_app = '',
        extParams = (jap.extParams && typeof jap.extParams === 'object') ? jap.extParams : '',
    ...
    //userAgent = "jdapp;iPhone;4.3.0;7.1;7ff475e854f125bceb9c40528243ab45dd899403;network/wifi;pv/5.1;psn/1111;psq/2222;ref/3333;pap/4444;usc/55555;usp/6666;umd/77777;utr/88888;adk/99999;ads/00000";
    if (userAgent) {
   		userAgent += ";";
   		// psn
   		var beginIndex = userAgent.indexOf("psn/")+4;
   		var endIndex = userAgent.indexOf(";", beginIndex);
   		preSession = userAgent.substring(beginIndex, endIndex);
    	// psq
    	beginIndex = userAgent.indexOf("psq/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		preSeqnum = userAgent.substring(beginIndex, endIndex);
   		// ref
   		beginIndex = userAgent.indexOf("ref/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		refererUrl = userAgent.substring(beginIndex, endIndex);
   		// pap
   		beginIndex = userAgent.indexOf("pap/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		pre_app = userAgent.substring(beginIndex, endIndex);
   		// usc
   		beginIndex = userAgent.indexOf("usc/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		utm_source = userAgent.substring(beginIndex, endIndex);
   		// usp
   		beginIndex = userAgent.indexOf("usp/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		utm_campaign = userAgent.substring(beginIndex, endIndex);
   		// umd
   		beginIndex = userAgent.indexOf("umd/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		utm_medium = userAgent.substring(beginIndex, endIndex);
   		// utr
   		beginIndex = userAgent.indexOf("utr/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		utm_term = userAgent.substring(beginIndex, endIndex);
   		// adk
   		beginIndex = userAgent.indexOf("adk/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		adsCookieName = userAgent.substring(beginIndex, endIndex);
   		// ads
   		beginIndex = userAgent.indexOf("ads/")+4;
   		endIndex = userAgent.indexOf(";", beginIndex);
   		ads = userAgent.substring(beginIndex, endIndex);
    }
    //alert("preSession="+preSession+"\npreSeqnum="+preSeqnum+"\nrefererUrl="+refererUrl+"\npre_app="+pre_app+"\nutm_source="+utm_source+"\nutm_campaign="+utm_campaign+"\nutm_medium="+utm_medium+"\nutm_term="+utm_term+"\nadsCookieName="+adsCookieName+"\nads="+ads);
	...
	window.lgPv = jaService.logPv; // 暴露lgPv方法,用于自定义pv上报
	// 点击流业务 ---------------------------------------end
	...
	function getUnionSeries() {
		...
		alert("getUnionSeries() -> " + json_str);
		return json_str;
	}
	function getAndroidUnionSeries() {
		var json_str = getUnionSeries();
		alert("getAndroidUnionSeries() -> " + json_str);
		try {
			window.AndriodPing.setSeriesUnion(json_str);
		} catch(e) {
		}
		return json_str;
	}
	...
	if (typeof jap.autoLogPv == 'undefined' || jap.autoLogPv) {
		// 上报pv
	    jaService.logPv();
	    
	    if (jap.anchorpvflag) {
		    window.addEventListener("hashchange", function() {
				jaCommonData.refererUrl=jaCommonData.currentUrl;
		    	jaCommonData.currentUrl=document.location.href;
		    	jaCommonData.account=(typeof jap.account != 'undefined') ? jap.account : store.getItem('pin');
		    	jaService.logPv();
		    });
	    }
    }
